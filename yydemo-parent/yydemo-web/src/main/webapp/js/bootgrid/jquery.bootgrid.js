    ...
    */
            ...
            var settings = {
                ...
                success: function(response)
                {
                    ...
                    if(response.total>0 && response.rows.length==0){
                        // 计算当前数据最后一页页码
                        var pager = Math.ceil(response.total/response.rowCount);
                        that.current = pager;
                        // 再次执行查询
                        request.current = pager;
                        // AJAX同步方法请求数据
                        $.ajax({
                            type:'POST',
                            data:request,
                            ...
                            success:function(data){
                                ...
                                response.rowCount = data.rowCount;
    ...
    Grid.defaults = {
        ...
        css: {
            ...
            dropDownMenuText: "dropdown-text yy_select", // must be a unique class name or constellation of class names within the actionDropDown
         ...
         **/
        templates: {
            actionButton: "<button class=\"btn btn-default glyphicon glyphicon-refresh\" type=\"button\" title=\"{{ctx.text}}\">{{ctx.content}}</button>",
            ...
            // row: "<tr>{{ctx.cells}}</tr>",
            row: "<tr{{ctx.attr}}>{{ctx.cells}}</tr>",              //这个属性来控制表格的每一行的颜色变化的样式
            // <span class=\"{{css.icon}} input-group-addon {{css.iconSearch}}\"></span><input type=\"text\" class=\"{{css.searchField}}\" placeholder=\"{{lbl.search}}\" />
            search: "<div class=\"{{css.search}}\"><div class=\"input-group\"></div></div>",
    ...
    {
        console.log(Grid.prototype.getTotalPageCount);
        // this.current = 1; // reset  默认设置刷新到第一页，不设置就刷新还在当前页
        loadData.call(this);
        return this;
    };
     ...
     * 刷新页面，加载第一页
     */
    Grid.prototype.repage = function()
    {
        this.current = 1; // reset  默认设置刷新到第一页，不设置就刷新还在当前页
        loadData.call(this);
        return this;
