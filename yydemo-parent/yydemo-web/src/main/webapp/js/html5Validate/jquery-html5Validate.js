...
(function($, undefined) {
    ...
    OBJREG.CHS = "^[\\u0391-\\uFFE5]+$";
    OBJREG["prompt"].chs = "请输入汉字";
    OBJREG.IP = "^(\\d{1,2}|1\\d\\d|2[0-4]\\d|25[0-5])\\.(\\d{1,2}|1\\d\\d|2[0-4]\\d|25[0-5])\\.(\\d{1,2}|1\\d\\d|2[0-4]\\d|25[0-5])\\.(\\d{1,2}|1\\d\\d|2[0-4]\\d|25[0-5])$";
    OBJREG["prompt"].ip = "IP格式不正确";
    OBJREG.QQ = "^[1-9]\\d{4,10}$";
    OBJREG["prompt"].qq = "QQ号码不正确";
    OBJREG.LOGINNAME = "^\\w+$";
    OBJREG["prompt"].loginname = "只允许英文字母、数字及下划线。";
    OBJREG.ENTITYNAME = "^([\\w]|[\\u4E00-\\u9FA5])+$";
    OBJREG["prompt"].entityname = "只允许英文字母、数字、下划线及汉字。";
    OBJREG.IDCARD = "^\\d{15}|\\d{}18$";//身份证
    OBJREG["prompt"].idcard = "请输入正确的身份证号";
    // OBJREG.PASSWORD = "^[a-zA-Z]\\w{5,17}$";//用户密码
    // OBJREG["prompt"].password = "请输入正确的密码格式：以字母开头，长度在6-18之间，只能包含字符、数字和下划线";
